...
def buscar_profesionales(especialidad: str | None = None, sesion: Session = Depends(obtener_sesion)):
    q = select(Profesional)
    if especialidad:
        q = q.where(Profesional.especialidad == especialidad)
    items = sesion.exec(q).all()
    return [ProfesionalOut(id=p.id, especialidad=p.especialidad, verificado=p.verificado) for p in items]
...
def solicitar_turno(datos: SolicitarTurnoIn, sesion: Session = Depends(obtener_sesion)):
    if not sesion.get(Profesional, datos.id_profesional):
        raise HTTPException(404, "Profesional inexistente")
    t = Turno(id=str(uuid.uuid4()), id_paciente=datos.id_paciente,
              id_profesional=datos.id_profesional, estado="pendiente",
...
def cambiar_estado_turno(datos: ConfirmarCancelarIn, sesion: Session = Depends(obtener_sesion)):
