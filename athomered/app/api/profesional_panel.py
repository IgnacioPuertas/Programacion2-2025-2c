...
def alta_perfil(datos: AltaPerfil, sesion: Session = Depends(obtener_sesion)):
    prof = sesion.exec(select(Profesional).where(Profesional.id_usuario == datos.id_usuario)).first()
    if not prof:
        raise HTTPException(404, "Profesional no encontrado")
    prof.matricula = datos.matricula
    prof.especialidad = datos.especialidad
    sesion.add(prof); sesion.commit(); sesion.refresh(prof)
...
def disponibilidad(datos: DisponibilidadIn, sesion: Session = Depends(obtener_sesion)):
    prof = sesion.get(Profesional, datos.id_profesional)
    if not prof:
        raise HTTPException(404, "Profesional no encontrado")
    prof.verificado = datos.disponible
    sesion.add(prof); sesion.commit()
...
def pendientes(id_profesional: str, sesion: Session = Depends(obtener_sesion)):
    items = sesion.exec(select(Turno).where((Turno.id_profesional == id_profesional) & (Turno.estado == "pendiente"))).all()
    return [TurnoOut(id=t.id, id_paciente=t.id_paciente, estado=t.estado) for t in items]
...
def decidir_turno(datos: DecidirTurnoIn, sesion: Session = Depends(obtener_sesion)):
    t = sesion.get(Turno, datos.id_turno)
    if not t:
        raise HTTPException(404, "Turno no encontrado")
    t.estado = "aceptado" if datos.decision == "aceptar" else "rechazado"
    sesion.add(t); sesion.commit(); sesion.refresh(t)
    bus.publicar(TurnoActualizado(
        id_turno=t.id,
...
def historial(id_profesional: str, sesion: Session = Depends(obtener_sesion)):
    items = sesion.exec(select(Turno).where(Turno.id_profesional == id_profesional)).all()
    return [HistorialOut(id=t.id, id_paciente=t.id_paciente, estado=t.estado) for t in items]
